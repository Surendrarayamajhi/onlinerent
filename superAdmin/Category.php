...
if (isset($_GET['activeId'])) {
    $catid_to_activate = $_GET['activeId'];

    // Perform the update operation to set isActive to 1
    $activate_query = "UPDATE category SET isActive = 1 WHERE id = $catid_to_activate";
    ...
    $catid_to_deactivate = $_GET['inactiveId'];

    // Assuming $conn object and your custom runQuery function

    // Perform the update operation to set isActive to 0
    $deactivate_query = "UPDATE category SET isActive = 0 WHERE id = $catid_to_deactivate";
...
<body id="body">
    <div class="css-container">
        ...
        <main class="px-3 py-3">
            ...
            <h4>Categories</h4>
            <a href="CategoryAdd.php"><button class="btn btn-primary mb-3">Add</button></a>
            <table id="myTable" class="table display">
                ...
                <tbody>
                    ...
                    foreach ($categoryLists as $categoryId => $category) {
                        ...
                        // Add action buttons
                        echo '<td>';
                        echo '<a href="CategoryEdit.php?catid=' . $categoryId . '" class="btn btn-warning btn-sm me-2">Edit</a>';

                        if ($isActive) {
                            echo '<a href="Category.php?inactiveId=' . $categoryId . '" class="btn btn-danger btn-sm me-2">Inactive</a>';
                        } else {
                            echo '<a href="Category.php?activeId=' . $categoryId . '" class="btn btn-success btn-sm me-2">Active</a>';
                        }
